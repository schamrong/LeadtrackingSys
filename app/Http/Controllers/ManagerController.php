    ...
    public function yourAgentCustomer()
    {
        ...
        if(Auth::user()->agent_code == $agent_code && Auth::user()->agent_code == $management_code)
        {
            try{
                ...
                $customers = DB::table('TAGENT_CUSTOMERS')->join('TAGENTS','TAGENT_CUSTOMERS.CREATE_BY','=','TAGENTS.IP_CODE')->select('TAGENT_CUSTOMERS.ID','TAGENT_CUSTOMERS.AGENT_CODE','TAGENT_CUSTOMERS.BRANCH_CODE','TAGENT_CUSTOMERS.FIRST_NAME','TAGENT_CUSTOMERS.LAST_NAME','TAGENT_CUSTOMERS.GENDER','TAGENT_CUSTOMERS.DOB','TAGENT_CUSTOMERS.CONTACT_NUMBER','TAGENT_CUSTOMERS.EMAIL','TAGENT_CUSTOMERS.ESTIMATE_INCOME','TAGENT_CUSTOMERS.RELATION_CUST','TAGENT_CUSTOMERS.CREATE_BY','TAGENT_CUSTOMERS.CREATE_DATE','TAGENT_CUSTOMERS.PROVINCE','TAGENT_CUSTOMERS.DISTRICT','TAGENT_CUSTOMERS.ADDRESS')->paginate(10);
            }catch(Exception $ex)
            {
                return view('error404',['message'=>$ex->getMessage()]);
            }
